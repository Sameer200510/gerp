...
export default function ApplicationDetail() {
  ...
  useEffect(() => {
    const fetchLead = async () => {
      try {
        const res = await fetch(`http://localhost:3001/api/admissions/${params.id}`);
  ...
  const handleApprove = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:3001/api/admissions/${params.id}/status`, {
        method: 'PUT',
  ...
  const handlePaymentCompleted = async () => {
    setIsProcessing(true);
    try {
      const res = await fetch(`http://localhost:3001/api/admissions/${params.id}/approve`, {
        method: 'POST',
      });
      if (!res.ok) throw new Error('Payment completion failed');
      
      // Refetch to get the newly generated Student Profile (ERP ID)
      const freshRes = await fetch(`http://localhost:3001/api/admissions/${params.id}`);
      if (freshRes.ok) {
        const freshData = await freshRes.json();
        setApplicant(freshData);
      } else {
